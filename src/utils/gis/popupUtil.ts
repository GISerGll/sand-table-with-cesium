 ...
 * @LastEditTime: 2024-01-04 18:13:06
...
export default class popupUtil {
  ...
  addPopup(feature: IFeature, dom: HTMLElement) {
    ...
    let screenSpaceEvent = cesiumStore.$state.event.get("PopupEvent");
    if (!CesiumDefined(screenSpaceEvent)) {
      screenSpaceEvent = this.viewer.scene.postRender.addEventListener(() => {
